    ...
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          num seconds = int.parse(item.dateOfRecommendation!.substring(item.dateOfRecommendation!.indexOf("=") + 1, item.dateOfRecommendation!.indexOf(",")));
          num nanoseconds = int.parse(item.dateOfRecommendation!.substring(item.dateOfRecommendation!.lastIndexOf("=") + 1, item.dateOfRecommendation!.indexOf(")")));

          DateTime date = DateTime.fromMillisecondsSinceEpoch(((seconds * 1000).toInt()) + (nanoseconds ~/ 1000000));
                ...
                children: [
                  ///
                    Expanded(
                      ...
                      child: InkWell(
                        onTap: (){
                          buildShowRestaurantDetails(context , item.restaurant! , heightM , txt_color);
                        },
                        child: Row(
                          children: [
                            ClipRRect(
                              borderRadius: BorderRadius.all(Radius.circular(10.0)),
                              child: Image.network("${item.restaurant?.photos?.first}",
                                  height: heightM * 2.5,
                                  width: heightM * 2.5,
                                  fit: BoxFit.fill),
                            ),
